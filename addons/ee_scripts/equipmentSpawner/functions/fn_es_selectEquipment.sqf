...
if (EE_Scripts_es_debug) then {systemChat format["Select random pool number: %1", _select]};
_eqm = _pool select _select;
scopeName "main";
_listed = [_eqm, _type] call EE_Scripts_fnc_es_inBlacklist;
if (_listed) then {
	if (EE_Scripts_es_debug) then {systemChat "Select another equipment down"};
	_i = _select -1;
  while {_i >= 0} do
	{
		_eqm = _pool select  _i;
		if (EE_Scripts_es_debug) then {systemChat format["Select pool number: %1", _i]};
		_listed = [_eqm, _type] call EE_Scripts_fnc_es_inBlacklist;
		if (!_listed) then {
			breakTo "main";
		};
	...
	if (EE_Scripts_es_debug) then {systemChat "Select another equipment up"};
	_i = _select +1;
...
{
  if (EE_Scripts_es_debug) then {systemChat format["Add to blacklist: %1", _x]};
  switch (_type) do {
    case "item": {
