...
if (_locked >= 2) then {
  ...
  _action = [_vehicle, ["Lock vehicle", {[_this select 3 select 0, _this select 3 select 1] call EE_Scripts_fnc_vs_toggleLock}, [_logic, _vehicle],1.5,false]] remoteExec ["addAction", -2, _vehicle];
  _vehicle lock 0;
}else{
  ...
  _action = [_vehicle, ["Unlock vehicle", {[_this select 3 select 0, _this select 3 select 1] call EE_Scripts_fnc_vs_toggleLock}, [_logic, _vehicle],1.5,false,false]] remoteExec ["addAction", -2, _vehicle];
  _vehicle lock 2;
};
