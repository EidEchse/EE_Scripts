...
while {true} do
{
    ...
    [1, "reloadingAmmobox", format["nearestObjects[%1, %2, %3]",_box, _reloading, _distance], EE_Scripts_ra_debug] call EE_Scripts_fnc_debug;
    _boxes = nearestObjects [_box, _reloading, _distance];
    _alive = false;
    {
      [1, "reloadingAmmobox", format["Found box %1 for reload",_x], EE_Scripts_ra_debug] call EE_Scripts_fnc_debug;
      if (alive _x) then {
        _alive = true;
      };
    } forEach _boxes;
    ...
    if (_alive) then
    {
      ...
      if (!isNil {_items}) then {
        ...
        {
          _cfgItem = _x;
          [1, "reloadingAmmobox", format["Check items %1",count (_boxItem select 0)], EE_Scripts_ra_debug] call EE_Scripts_fnc_debug;
          _found = false;
          for [{_i=0},{_i < count (_boxItem select 0)},{_i=_i+1}] do
          {
            [1, "reloadingAmmobox", format["Compare Item %1 == %2",_cfgItem select 0, (_boxItem select 0 select _i)], EE_Scripts_ra_debug] call EE_Scripts_fnc_debug;
            if ((_cfgItem select 0) == (_boxItem select 0 select _i)) then
            {
              [0, "reloadingAmmobox", format["Item %1 found in box",_cfgItem], EE_Scripts_ra_debug] call EE_Scripts_fnc_debug;
              _found = true;
      ...
      if (!isNil {_magazines}) then {
        {
          ...
          _cfgMagazin = _x;
          [1, "reloadingAmmobox", format["Check magazine %1",count (_boxMagazin select 0)], EE_Scripts_ra_debug] call EE_Scripts_fnc_debug;
          _found = false;
          for [{_i=0},{_i < count (_boxMagazin select 0)},{_i=_i+1}] do
          {
            [1, "reloadingAmmobox", format["Compare magazine %1 == %2",_cfgMagazin select 0, (_boxMagazin select 0 select _i)], EE_Scripts_ra_debug] call EE_Scripts_fnc_debug;
            if ((_cfgMagazin select 0) == (_boxMagazin select 0 select _i)) then {
              [0, "reloadingAmmobox", format["Magazine %1 found in box",_cfgMagazin], EE_Scripts_ra_debug] call EE_Scripts_fnc_debug;
              _found = true;
