...
[0, "vehicleSpawner", format["Select random pool number: %1", _select], EE_Scripts_vs_debug] spawn EE_Scripts_fnc_debug;
_vehicleName = _pool select _select;
scopeName "main";
_listed = [_vehicleName, _type] call EE_Scripts_fnc_vs_inBlacklist;
if (_listed) then {
	[0, "vehicleSpawner", "Select another equipment down"] spawn EE_Scripts_fnc_debug;
	_i = _select -1;
  while {_i >= 0} do
	{
		_vehicleName = _pool select  _i;
		[0, "vehicleSpawner", format["Select pool number: %1", _i], EE_Scripts_vs_debug] spawn EE_Scripts_fnc_debug;
		_listed = [_vehicleName, _type] call EE_Scripts_fnc_vs_inBlacklist;
		if (!_listed) then {
			breakTo "main";
		};
	...
	[0, "vehicleSpawner", "Select another vehicle up"] spawn EE_Scripts_fnc_debug;
	_i = _select +1;
...
[0, "vehicleSpawner", format["Add to blacklist: %1", _vehicleName], EE_Scripts_vs_debug] spawn EE_Scripts_fnc_debug;
switch _type do {
	case "aa": {
