...
if (!isNil {_objects}) then
{
  _found = false;
  {
    ...
    if (_x getVariable ["EE_Scripts_is_selectiveArsenal", false]) then
    {
      if (!_found) then {
        ...
        {
          {
            [0, "selectiveArsenal", format["Add backpack: %1", _list select 0], EE_Scripts_sa_debug] spawn EE_Scripts_fnc_debug;
            [_arsenal, _x, true] call BIS_fnc_addVirtualBackpackCargo;
            _cfg = (configfile >> "CfgWeapons" >> _x);
            [0, "selectiveArsenal", format ["Class %1 config %2",_x, _cfg], EE_Scripts_sa_debug] spawn EE_Scripts_fnc_debug;
            ...
            [{hint parseText (_title + _picture + _footer);},"BIS_fnc_spawn",true,true] call BIS_fnc_MP;
          } forEach _list;
          sleep 5;
          [{hint "";},"BIS_fnc_spawn",true,true] call BIS_fnc_MP;
          sleep 0.5;
        };
        ...
        {
          {
            [0, "selectiveArsenal", format["Add item: %1", _list select 0], EE_Scripts_sa_debug] spawn EE_Scripts_fnc_debug;
        ...
        {
          {
            [0, "selectiveArsenal", format["Add weapon: %1", _list select 0], EE_Scripts_sa_debug] spawn EE_Scripts_fnc_debug;
  ...
  [{hint "Selectiv Arsenal items added!";},"BIS_fnc_spawn",true,true] call BIS_fnc_MP;
  deleteVehicle _box;
};
true
