...
if (_nextRespawn == 0) then
{
  ...
  {
    [_box, [format ["!!! DELETE ALL UNITS !!!"], {[_this select 3] call EE_Scripts_fnc_us_deleteUnits}, _logic, 1.7, false, false]] remoteExec ["addAction", [0,-2] select isDedicated, _box];
  };
}else{
...
[0, "unitSpawner", format["Units that can be spawned: %1", _units], EE_Scripts_us_debug] spawn EE_Scripts_fnc_debug;
{
  ...
  [_box, [format ["Spawn: %1", _displayName], {[_this select 3 select 0, _this select 3 select 1, _this select 1] call EE_Scripts_fnc_us_spawnUnit},[_logic, _x],1.4,false,false,"","[_target] call EE_Scripts_fnc_us_respawnsLeft"]] remoteExec ["addAction", [0,-2] select isDedicated, _box];
} forEach _units;
